...
class Sample:
    ...
    def from_swe_bench(cls, benchmark: dict[str, str]) -> Sample:
        ...
        return cls(
            ...
            test_command=(
                "" 
                if not benchmark.get("FAIL_TO_PASS") 
                else "pytest " + " ".join(json.loads(benchmark["FAIL_TO_PASS"]))
            ),
            PASS_TO_PASS=(
                "" 
                if not benchmark.get("PASS_TO_PASS") 
                else "pytest " + " ".join(json.loads(benchmark["PASS_TO_PASS"]))
            ),
        )
