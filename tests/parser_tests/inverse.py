...
async def verify_inverse(parser):
    ...
    parsedLLMResponse = ParsedLLMResponse(
        ...
        file_edits=[
            FileEdit(
                file_path=PosixPath(f"{cwd}/test.txt"),
                replacements=[
                    Replacement(
                        starting_line=1,
                        ending_line=1,
            ...
            FileEdit(
                file_path=PosixPath(f"{cwd}/delete.txt"),
                replacements=[
            ...
            FileEdit(
                file_path=PosixPath(f"{cwd}/create.txt"),
                replacements=[
                    Replacement(
                        starting_line=0,
                        ending_line=0,
            ...
            FileEdit(
                file_path=PosixPath(f"{cwd}/file1.txt"),
                replacements=[
                    Replacement(
                        starting_line=1,
                        ending_line=1,
                ...
                is_creation=False,
                is_deletion=False,
                rename_file_path=PosixPath("file2.txt"),
            ),
        ],
    )
