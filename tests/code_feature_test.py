...
def test_split_file_into_intervals(temp_testbed: Path, mock_session_context: SessionContext):
...
def test_ref_method(temp_testbed: Path, mock_session_context: SessionContext):
    test_file = Path(temp_testbed) / "test_file.py"
    test_file.write_text("\n".join([""] * 10))
    expected = mock_session_context.cwd.joinpath("test_file.py:2-4,6-8")
