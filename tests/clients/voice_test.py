...
class TestTranscriber(unittest.TestCase):
    @patch("mentat.terminal.voice.transcriber.sd")
    @patch("mentat.terminal.voice.transcriber.WhisperModel")
    def test_transcriber_with_frozen_timestamp(self, mock_whisper_model, mock_sd):
        mock_buffer = Mock()
        mock_buffer.text = ""
        ...
        final_segment = [
            Mock(words=[Mock(word="elephant ", start=1.1, end=1.5)], end=2.0)
        ]
        ...
        self.assertEqual(mock_buffer.text, "")
        transcriber.process_audio(in_data, CHUNK, time_info, 0)
        self.assertEqual(mock_buffer.text, "one 2")
        ...
        self.assertEqual(mock_buffer.text, "one two three")
